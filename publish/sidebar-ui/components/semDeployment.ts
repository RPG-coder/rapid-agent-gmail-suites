...
function onVerifyDeploymentSEM(e: any) {
  ...
  if (manualUrl && manualUrl.startsWith("https://")) {
    ...
    return CardService.newActionResponseBuilder()
      .setNavigation(CardService.newNavigation().updateCard(createSEMDeploymentCard()))
      .setNotification(CardService.newNotification().setText("Service URL updated manually! Now you can Login to MongoDB from the Home screen."))
      .build();
  }
  ...
  try {
    ...
    if (response.getResponseCode() === 200) {
      ...
      if (matchedService) {
        ...
        return CardService.newActionResponseBuilder()
          .setNavigation(CardService.newNavigation().updateCard(createSEMDeploymentCard()))
          .setNotification(CardService.newNotification().setText("Cloud Run service verified successfully! Now you can Login to MongoDB from the Home screen."))
          .build();
      } else {
        return CardService.newActionResponseBuilder()
      ...
      if (response.getResponseCode() === 403) {
        return CardService.newActionResponseBuilder()
          .setNotification(CardService.newNotification().setText("Verification Error (403): Ensure 'Cloud Run Admin API' is enabled and you have 'Cloud Run Viewer' role."));
      }
      return CardService.newActionResponseBuilder()
...
function showSEMDeployment(e: any) {
  return CardService.newActionResponseBuilder()
    .setNavigation(CardService.newNavigation().pushCard(createSEMDeploymentCard()))
    .build();
}
